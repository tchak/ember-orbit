...
export class HasOneRelation extends Relation {
  get value(): Model | null {
    return (
      this.owner.store.cache.relatedRecord(this.owner.identity, this.name) ||
      null
    );
  }

  query(options?: object): FindRelatedRecordQueryBuilder {
    return this.owner.store.findRelatedRecord(
      this.owner.identity,
      this.name,
      options
    );
...
export class HasManyRelation extends Relation {
  get value(): Model[] {
    const records =
      this.owner.store.cache.relatedRecords(this.owner.identity, this.name) ||
      [];
  ...
  query(options?: object): FindRelatedRecordsQueryBuilder {
    return this.owner.store.findRelatedRecords(
      this.owner.identity,
      this.name,
      options
    );
